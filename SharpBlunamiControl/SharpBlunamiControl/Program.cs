...
namespace SharpBlunamiControl
{
    ...
    internal class BlunamiControl
    {
        ...
        static void Main(string[] args)
        {
            ...
            BlunamiControl.availablePorts = SerialPort.GetPortNames();

            if (BlunamiControl.availablePorts.Length != 0)
            {
                Console.WriteLine("Please enter the correct COM Port to use:");

                for (int i = 0; i < BlunamiControl.availablePorts.Length; i++)
                {
                    Console.WriteLine("[" + i.ToString() + "] " + BlunamiControl.availablePorts[i]);
                }
                ...
                if(keyEntered != null)
                {
                    ...
                    {
                        BlunamiControl.comPortIndex = int.Parse(keyEntered.KeyChar.ToString());
                        Console.WriteLine("Using COM port: " + BlunamiControl.availablePorts[BlunamiControl.comPortIndex]);

                        BlunamiControl.serialPort = new SerialPort(BlunamiControl.availablePorts[BlunamiControl.comPortIndex], 9600, Parity.None, 8, StopBits.One);
            ...
            {
                BlunamiControl.BlunamiBLESetup(BlunamiControl.watcher);
        ...
        private async void BlunamiBLESetup(BluetoothLEAdvertisementWatcher watcher)
        {
        ...
        private async void OnAdvertisementReceived(BluetoothLEAdvertisementWatcher watcher, BluetoothLEAdvertisementReceivedEventArgs eventArgs)
        {
            ...
            //{
                ...
                Console.WriteLine(string.Format("[{0}]: type={1}, rssi={2}, name={3}, manufacturerData=[{4}]",
